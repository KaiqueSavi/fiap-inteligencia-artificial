...
# ---------------------------------------------------------------------------
def salvar_backup_json(talhoes, caminho="src/backup_talhoes.json"):
...
def carregar_backup_json(caminho="src/backup_talhoes.json"):
...
# ---------------------------------------------------------------------------
def exportar_relatorio_txt(talhoes, caminho="output/relatorio.txt"):
    ...
    with open(caminho, "w", encoding="utf-8") as arquivo:
        ...
        arquivo.write(
            f"{'ID':<6}{'NOME':<20}{'ÁREA(ha)':>10}{'TIPO':>12}"
            f"{'ESP.(t)':>10}{'COL.(t)':>10}"
        ...
        for t in talhoes:
            ...
            id_mostrado = (
                f"O{t['id_oracle']}"
                if t.get("id_oracle") is not None
                else f"L{t['id_local']}"
            )
            arquivo.write(
        ...
        pct_medio = (
            (total_perda / total_esperado * 100.0) if total_esperado > 0 else 0.0
        )
